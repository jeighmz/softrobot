...
floor = box(pos = vector(0,0,0), length = 1000, width = 1000, height = 1)

v0 = args.v
p0 = vector(0,0,0)

direction= vector(cos(args.theta), sin(args.theta), sin(args.theta))
...
while t<100:
    ...
    if (ball.pos.y - ball.radius <= floor.pos.y + floor.height/2 and (-floor.length/2< ball.pos.x <floor.length/2) and (-floor.width/2< ball.pos.x <floor.width/2) ):
        ball.p.y = -ball.p.y
